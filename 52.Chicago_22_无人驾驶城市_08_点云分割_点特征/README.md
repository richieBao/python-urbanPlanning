...
目前三维点云数据的处理库日益完善，大部分数据处理无需自行从新编写代码。 点云数据的分割方法也丰富多彩， 不同的方法对原始数据处理的方式也基本会不同，主要有几种方式，一是将三维数据投影到二维平面，或直接球面投影为矩形图像，或转换为极坐标的方式；再者就是寻找三维点云本身的关系，例如使用体素（voxel），或者近似平面法线方向，以及应用PyTorch Geometric(PyG)建立图结构，使用图神经网络等。在下述的代码中迁移与调整已有研究成果（代码或算法），来比较不同方式下点云数据的处理。

代码中包含有多个点云数据转动画.gif的代码，以及.gif文件优化的程序，可以存储车行路径所有信息，方便观察，当然可以直接应用Open3D库运行代码交互查看。

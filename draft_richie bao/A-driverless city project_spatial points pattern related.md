...
激光雷达在每个位置点旋转扫描，提取的特征点位于各个方向上，因此尝试在每个位置点划分36个方向区域（数量大小在进一步分析中可以为连续数量切分，例如4份，8份...72份），分析landmarks位置对PHMI值的影响。同时应用三种数据类型，一是含距离值，数据示例：
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTI4MDQzMDgxOSwtMTM0NDA4MzAxNSwzNT

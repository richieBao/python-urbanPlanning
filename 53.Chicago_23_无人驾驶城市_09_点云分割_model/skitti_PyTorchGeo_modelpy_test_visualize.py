...
def test_o3dVisualization(loader, model_path=''):
    ...
    for data in loader:
        ...
        # o3d.io.write_point_cloud(r"E:\dataset\results\dcp_graph_prediction_pcd\%s.pcd"%j, pcd)  #o3d.io.write_point_cloud(r"E:\dataset\results\PyTorchGeo_poiNet_pcd_img\pcd\%s.pcd"%j, pcd) 
        j+=1
        
    
def IoU_test(model_root,data_path, imageset = 'test',split_ratio=[0.7,0.2,0.1],suffix='.pth',epoch_num=2):
    ...
    import pandas as pd
    import statistics
    model_files=[os.path.join(model_root,f) for f in listdir(model_root) if isfile(join(model_root, f)) and f[-4:]==suffix]    
    
    
    Iou_results={}
    i=0
    for mod in tqdm(model_files):
        iou_lst=[]
        for epoch in range(1,epoch_num):
            iou = test(test_loader,model_path=mod)
            iou_lst.append(iou)
        
        iou_mean=statistics.mean(iou_lst)
        print("\n")
        print('idx:{},model: {}, Test IoU: {:.4f}'.format(i,os.path.split(mod)[-1], iou_mean))        
        Iou_results[os.path.split(mod)[-1]]=iou_mean
        # if i==0:break
        # i+=1
    # print("_"*50)
    # print(Iou_results)    
    return pd.DataFrame({'file_name':list(Iou_results.keys()),'IoU':list(Iou_results.values())})
...
if __name__ == '__main__':
    ...
    model_path=r'C:\Users\richi\omen-richiebao\omen-code\Chicago_code\dcp_cloudPoints_processing\model_save\skitti_PyTorchGeo_epoch_3599_Acc_0.7523.pth' #skitti_PyTorchGeo_epoch_449_Acc_0.8343.pth
    # for epoch in range(1,2):
    #     iou = test(test_loader,model_path=model_path)
    #     print('Epoch: {:02d}, Test IoU: {:.4f}'.format(epoch, iou))


    # model_root=r'C:\Users\richi\omen-richiebao\omen-code\Chicago_code\dcp_cloudPoints_processing\model_save'
    # IoUs=IoU_test(model_root=model_root,data_path=dataset_path, imageset = 'test',split_ratio=[0.7,0.2,0.1],suffix='.pth',epoch_num=2)



    
    # pd.set_option("display.max_rows", None, "display.max_columns", None)
    # IoUs_sort=IoUs.sort_values(by='IoU')
    # IoUs_sort.to_pickle(r'.\results\IoU.pkl')
    # IoUs_sort.to_csv(r'.\results\IoU.csv')    
    ...
    dcp_data_path=r'E:\dataset\results\graph_dataset_batch'
    dcp_loader=dataloader(data_path=dcp_data_path, imageset = 'test',split_ratio=[0,0,1]) 
    test_o3dVisualization(dcp_loader,model_path=model_path)

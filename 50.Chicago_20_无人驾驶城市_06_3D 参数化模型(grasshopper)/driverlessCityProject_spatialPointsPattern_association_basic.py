...
def readMatLabFig_PHMI_A(PHMI_fn,LandmarkMap_dic):
    ...
    PHMI_dic={} #extract the .fig format file of MatLab 提取MatLab的.fig值
...
#one-dimentional convolution segmentation curve jump point 1维卷积切分曲线跳变点
def con_1_dim(data,loc_x):
    kernel_conv=[-1,2,-1] #convolution kernel 卷积核，类似2维提取图像的边缘
    ...
    #give different colors to print according to the curve of the jump point 根据跳变点切分的曲线赋予不同的颜色打印
...
#merge landmarks, locations, and curve jitter curve to print 合并landmarks，locations和曲线跳变点切分，打印
...
def location_landmarks_network(targetPts_idx,locations,landmarks):   
    ...
    plt.show()
    return G
#interactive network diagram 网络交互图表
def interactiveG(G):
